/* @generated by ResGraph named schema */

@@warning("-27-32")

open ResGraph__GraphQLJs

let typeUnwrapper: ('src) => 'return = %raw(`function typeUnwrapper(src) { if (src == null) return null; if (typeof src === 'object' && src.hasOwnProperty('_0')) return src['_0']; if (typeof src === 'object' && src.hasOwnProperty('VAL')) return src['VAL']; return src;}`)
let inputUnionUnwrapper: ('src, array<string>, array<string>) => 'return = %raw(`function inputUnionUnwrapper(src, inlineRecordTypenames, emptyPayloadTypenames) {
      if (src == null) return null;

      let targetKey = null;
      let targetValue = null;

      Object.entries(src).forEach(([key, value]) => {
        if (value != null) {
          targetKey = key;
          targetValue = value;
        }
      });

      if (targetKey != null && targetValue != null) {
        let tagName = targetKey.slice(0, 1).toUpperCase() + targetKey.slice(1);

        if (inlineRecordTypenames.includes(tagName)) {
          return Object.assign({ TAG: tagName }, targetValue);
        }

        if (emptyPayloadTypenames.includes(tagName)) {
          return tagName;
        }

        return {
          TAG: tagName,
          _0: targetValue,
        };
      }

      return null;
    }
    `)
let resolveInterfaceTypename: ('src, array<string>, string, string) => string = %raw(`function resolveInterfaceTypename(src, allowedTypenames, interfaceName, interfaceResolverTypeName) {
      if (allowedTypenames.length === 1) {
        return allowedTypenames[0];
      }

      if (src != null && typeof src === "object") {
        let tag = src.TAG;

        if (typeof tag === "string" && allowedTypenames.includes(tag)) {
          return tag;
        }

        if (typeof tag === "string") {
          throw new Error(
            "Panic! Interface " +
              interfaceName +
              " resolveType got unexpected TAG " +
              JSON.stringify(tag) +
              ". Expected one of " +
              allowedTypenames.join(", ") +
              ".",
          );
        }
      }

      throw new Error(
        "Panic! Interface " +
          interfaceName +
          " resolveType expected a tagged value from " +
          interfaceResolverTypeName +
          ", but got an untagged value. Use " +
          interfaceResolverTypeName +
          " for interface return values instead of the bare interface record type.",
      );
    }
    `)

type inputObjectFieldConverterFn
external makeInputObjectFieldConverterFn: ('a => 'b) => inputObjectFieldConverterFn = "%identity"

let applyConversionToInputObject: ('a, array<(string, inputObjectFieldConverterFn)>) => 'a = %raw(`function applyConversionToInputObject(obj, instructions) {
  if (instructions.length === 0) return obj;
  let newObj = Object.assign({}, obj);
  instructions.forEach(instruction => {
    let value = newObj[instruction[0]];
    newObj[instruction[0]] = instruction[1](value);
  })
  return newObj;
}`)


let i_Entity: ref<GraphQLInterfaceType.t> = Obj.magic({"contents": null})
let get_Entity = () => i_Entity.contents
let t_Query: ref<GraphQLObjectType.t> = Obj.magic({"contents": null})
let get_Query = () => t_Query.contents
let t_SharedItem: ref<GraphQLObjectType.t> = Obj.magic({"contents": null})
let get_SharedItem = () => t_SharedItem.contents

let interface_Entity_resolveType = (v: PublicSchema__Interface_entity.Resolver.t) => resolveInterfaceTypename(v, ["SharedItem"], "Entity", "PublicSchema__Interface_entity.Resolver.t")

i_Entity.contents = GraphQLInterfaceType.make({
  name: "Entity",
  description: ?(None),
  interfaces: [],
  fields: () => {
    "id": {
      typ: Scalars.string->Scalars.toGraphQLType->nonNull,
      description: ?(None),
      deprecationReason: ?(None),
    }
  }->makeFields,
  resolveType: GraphQLInterfaceType.makeResolveInterfaceTypeFn(interface_Entity_resolveType)
})
t_Query.contents = GraphQLObjectType.make({
  name: "Query",
  description: ?(None),
  interfaces: [],
  fields: () => {
    "entity": {
      typ: get_Entity()->GraphQLInterfaceType.toGraphQLType->nonNull,
      description: ?(None),
      deprecationReason: ?(None),
      resolve: makeResolveFn((src, args, ctx, info) => {let src = typeUnwrapper(src); PublicInterface.entity(src)})
    },
    "publicValue": {
      typ: Scalars.string->Scalars.toGraphQLType->nonNull,
      description: ?(None),
      deprecationReason: ?(None),
      resolve: makeResolveFn((src, args, ctx, info) => {let src = typeUnwrapper(src); Public.publicValue(src, ~ctx=ctx)})
    },
    "sharedItem": {
      typ: get_SharedItem()->GraphQLObjectType.toGraphQLType->nonNull,
      description: ?(None),
      deprecationReason: ?(None),
      resolve: makeResolveFn((src, args, ctx, info) => {let src = typeUnwrapper(src); Public.sharedItem(src)})
    }
  }->makeFields
})
t_SharedItem.contents = GraphQLObjectType.make({
  name: "SharedItem",
  description: ?(None),
  interfaces: [get_Entity()],
  fields: () => {
    "id": {
      typ: Scalars.string->Scalars.toGraphQLType->nonNull,
      description: ?(None),
      deprecationReason: ?(None),
      resolve: makeResolveFn((src, _args, _ctx, _info) => {let src = typeUnwrapper(src); src["id"]})
    },
    "label": {
      typ: Scalars.string->Scalars.toGraphQLType->nonNull,
      description: ?(None),
      deprecationReason: ?(None),
      resolve: makeResolveFn((src, _args, _ctx, _info) => {let src = typeUnwrapper(src); src["label"]})
    }
  }->makeFields
})

let schema = GraphQLSchemaType.make({
  "query": get_Query(),
  "types": [
    get_Query()->GraphQLObjectType.toGraphQLType,
    get_SharedItem()->GraphQLObjectType.toGraphQLType,
    get_Entity()->GraphQLInterfaceType.toGraphQLType
  ]
})
