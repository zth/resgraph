...
let scalar_Timestamp = GraphQLScalar.make({name: "Timestamp", description: ?(None), specifiedByURL: ?(None)})
let scalar_Timestamp2 = GraphQLScalar.make({name: "Timestamp2", description: ?(None), specifiedByURL: ?(None)})
...
let enum_TimestampFormat = GraphQLEnumType.make({
  ...
  values: {
    ...
    "HumanReadable": {GraphQLEnumType.value: "HumanReadable", description: ?(None), deprecationReason: ?(None)},
  }->makeEnumValues,
})
...
let inputUnion_FindShopInput: ref<GraphQLInputObjectType.t> = Obj.magic({"contents": null})
let get_FindShopInput = () => inputUnion_FindShopInput.contents
let inputUnion_FindShopInput_conversionInstructions = []
let input_FindShopInputByAddress: ref<GraphQLInputObjectType.t> = Obj.magic({"contents": null})
...
let input_Coordinates: ref<GraphQLInputObjectType.t> = Obj.magic({"contents": null})
...
inputUnion_FindShopInput_conversionInstructions->Array.pushMany([
  ...
  (
    "byAddress",
    makeInputObjectFieldConverterFn((v) => (switch v->Nullable.toOption { | None => None | Some(v) => v->applyConversionToInputObject(input_FindShopInputByAddress_conversionInstructions)->Some}))
  ),
  (
    "byCoordinates",
    makeInputObjectFieldConverterFn((v) => (switch v->Nullable.toOption { | None => None | Some(v) => v->applyConversionToInputObject(input_Coordinates_conversionInstructions)->Some}))
  ),
])

let interface_HasName_resolveType = (v: Interface_hasName.Resolver.t) => resolveInterfaceTypename(v, ["User"], "HasName", "Interface_hasName.Resolver.t")
...
t_Query.contents = GraphQLObjectType.make({
  ...
  fields: () => {
    "currentTime": {
      ...
      description: ?(None),
      deprecationReason: ?(None),
      resolve: makeResolveFn((src, args, ctx, info) => {let src = typeUnwrapper(src); GraphQLSchema.currentTime(src)})
    },
    "currentTimeFloat": {
      typ: Scalars.string->Scalars.toGraphQLType,
      description: ?(None),
    ...
    "me": {
      ...
      deprecationReason: ?(None),
      args: dict{
        "onlyIfAvailable": ({typ: Scalars.boolean->Scalars.toGraphQLType->nonNull}: arg)
      }->makeArgsDict,
      resolve: makeResolveFn((src, args, ctx, info) => {let src = typeUnwrapper(src); GraphQLSchema.me(src, ~onlyIfAvailable=args["onlyIfAvailable"])})
    },
    "shop": {
      typ: Scalars.string->Scalars.toGraphQLType,
      description: ?(None),
      deprecationReason: ?(None),
      args: dict{
        "input": ({typ: get_FindShopInput()->GraphQLInputObjectType.toGraphQLType->nonNull}: arg)
      }->makeArgsDict,
      resolve: makeResolveFn((src, args, ctx, info) => {let src = typeUnwrapper(src); GraphQLSchema.shop(src, ~input=args["input"]->applyConversionToInputObject(inputUnion_FindShopInput_conversionInstructions)->inputUnionUnwrapper(["ByAddress"], []))})
    }
  }->makeFields
})
t_Subscription.contents = GraphQLObjectType.make({
  ...
  fields: () => {
    "countdown": {
      ...
      subscribe: makeResolveFn((src, args, ctx, info) => {let src = typeUnwrapper(src); GraphQLSchema.countdown(src)})
    }
  }->makeFields
})
