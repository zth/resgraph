...
You can use interfaces as return types as well in your schema. Any time an interface is in an output position, use the autogenerated `Resolver.t` type for that interface from `Interface_<interfaceName>.res`.

This applies both to field resolvers on `Query`/`Mutation` and to fields nested inside other `@gql.type` records.

For example, for a field resolver:
...
@gql.type
type profileCard = {
  @gql.field featured: Resolver.t
}

@gql.field
let profileCard = (_: query): profileCard => {
  featured: User({
    name: "Alice",
    age: 42,
  }),
...
Do not use the bare interface record type for interface return values. The bare interface record type is for implementing the shared field shape and for interface field resolvers. Output values must use `Interface_<interfaceName>.Resolver.t`.
