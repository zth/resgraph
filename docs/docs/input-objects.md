...
Input object fields can also use ReScript's `@as` attribute when the GraphQL field name should differ from the ReScript record field name. This is useful when the GraphQL input needs a field name that is reserved in ReScript.
