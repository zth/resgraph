...
let id = (
  ...
  `${typename->ResGraphSchemaAssets.node_typenameToString}:${node.id}`
...
### Compressing type names with `NodeInterfaceTypeMap`
...
let nodeTypeMap = typeMap->NodeInterfaceTypeMap.make(~valueToString=Int.toString)
```

A few things to distill.

1. `node_typeMap<'value>` is generated, and is a record type that expects you to fill in all types implementing the Node interface, and what `'value` you want to map them to. Here we're mapping to `int`.
...
3. We then call `NodeInterfaceTypeMap.make` with our type map, and also pass it a `~valueToString` function that shows how to turn `'value` into a string.

Now we have a `NodeInterfaceTypeMap.t` that we can leverage for encoding and decoding the type names in our `id`. Let's update our examples:
...
let nodeTypeMap =
  typeMap->NodeInterfaceTypeMap.make(~valueToString=Int.toString)

@gql.field
let id = (node: NodeInterface.node, ~typename: node_implementedBy) => {
  let typenameAsString =
    nodeTypeMap->NodeInterfaceTypeMap.getStringifiedValueByType(typename)
...
Let's show how we can leverage `NodeInterfaceTypeMap` as we decode IDs in the `node` field function:
...
> => {
  ...
  | [compressedTypename, id] =>
    switch nodeTypeMap->NodeInterfaceTypeMap.getTypeByStringifiedValue(compressedTypename) {
    | None => None
    | Some(User) =>
      switch await ctx.dataLoaders.userById.load(~userId=id) {
      | Ok(user) => Some(User(user))
...
We're now using `NodeInterfaceTypeMap.getTypeByStringifiedValue` to go directly from a compressed string value in the `id` (like `"1"` for the integer `1` representing `User`) to its underlying type, if it can be mapped.
