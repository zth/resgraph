          ...
          ~diagnostic:
            {
              ...
              message =
                Printf.sprintf
                  "The variant case `%s` of the GraphQL enum variant `%s` has \
                   a payload. Variants tagged as @gql.enum can only have cases \
                   without payloads. "
...
let rec findGraphQLType ~(env : SharedTypes.QueryEnv.t)
  ...
  let rec findTyp t =
    ...
    | Tlink te | Tsubst te | Tpoly (te, []) -> findTyp te
    | _ -> t
  in
  ...
  if isSubscription then (
    ...
    | Tconstr (Path.Pident {name = "option"}, [unwrappedType], _) -> (
      let inner =
        findGraphQLType ~env ~schemaState ~debug ~full ~typeContext
          unwrappedType
      in
      match inner with
      | None -> None
      ...
      | _ -> (
        ...
        | Some (env, {item}) -> (
          ...
          | ( Some ObjectType,
              {
                attributes;
                name = ("query" | "mutation" | "subscription") as name;
                kind = Abstract None;
              } ) ->
            let id = name in
          ...
          | Some (InterfaceResolver {interfaceId}), {kind = Variant _} ->
            Some
              (GraphQLInterface
                 {
                   id = interfaceId;
                   displayName = capitalizeFirstChar interfaceId;
                 })
          | Some Scalar, {name = "t"} ->
                 ...
                 ~diagnostic:
                   {
                     ...
                     message =
                       Printf.sprintf
                         "The type `%s` does not represent a valid GraphQL \
                          type."
                         item.name;
                   };
            None)
             ...
             ~diagnostic:
               {
                 ...
                 message =
                   Printf.sprintf
                     "Found an object in a place where it cannot be inferred \
                      as a GraphQL type. You can only use objects in union \
                      members.";
               };
        None)
    | Tvariant {row_fields} -> (
             ...
             ~diagnostic:
               {
                 ...
                 message =
                   Printf.sprintf
                     "Found a polyvariant in a place where it cannot be \
                      inferred as a GraphQL type. You can only use \
                      polyvariants in resolver arguments and return types.";
               };
        None
      | ArgumentType _ | ReturnType _ | ObjectField _ ->
        ...
        let syntheticTypeNameParts =
          ...
          | ObjectField {objectTypeName; fieldName} ->
               ...
               ~diagnostic:
                 {
                   ...
                   message =
                     Printf.sprintf
                       "Tried to infer returned polyvariant as either an enum \
                        or variant, but the polyvariant contained one or \
                        more:\n\n\
                        %s"
                       (errors
                       |> List.map (fun err ->
                     ...
                     message =
                       Printf.sprintf
                         "Tried to infer returned polyvariant as union but it \
                          ...
                          Polyvariants to be inferred as unions can only \
                          contain constructors with payloads that refer to \
                          valid GraphQL types.";
                   };
            None)
                                       ...
                                       message =
                                         Printf.sprintf
                                           "Found an invalid union member. \
                                            Unions can only contain objects or \
                                            interfaces.";
                                     };
                              ("", "")
                          in
                   ...
                   ~diagnostic:
                     {
                       loc =
                         (match loc with
                         | None ->
                           Location.in_file (env.file.moduleName ^ ".res")
                         | Some loc -> loc);
                       fileUri = env.file.uri;
                       message =
                         Printf.sprintf
                           "Tried to infer returned polyvariant as union but \
                            it is in an invalid position. Currently, \
                            polyvariants with payloads can only be inferred as \
                            unions as retrun types of resolvers.";
                     };
              None)
          else None)
    | _ ->
             ...
             ~diagnostic:
               {
                 ...
                 message =
                   Printf.sprintf
                     "Field `%s` is is not a valid GraphQL type. All fields of \
                      a @gql.inputObject must be valid GraphQL types."
                     field.fname.txt;
               };
        None
      | Some typ ->
...
and variantCasesToUnionValues ~env ~debug ~schemaState ~full ~ownerName
  ...
  |> List.filter_map (fun (case : SharedTypes.Constructor.t) ->
      ...
      | Args [(typ, _)] -> (
        match
          findGraphQLType ~debug ~loc:case.cname.loc ~env ~schemaState ~full typ
        with
        | Some (GraphQLObjectType {id; displayName}) ->
          Some
            {
            ...
            ~diagnostic:
              {
                ...
                message =
                  Printf.sprintf
                    "The payload of the variant case `%s` of the GraphQL union \
                     variant `%s` is not a GraphQL object. The payload needs \
                     to be a single type representing a GraphQL object, \
          ...
          ~diagnostic:
            {
              ...
              message =
                Printf.sprintf
                  "The payload of the variant case `%s` of the GraphQL union \
                   variant `%s` is not a single payload. The payload needs to \
                   be a single type representing a GraphQL object, meaning \
                   it's annotated with @gql.type."
                  case.cname.txt (case.typeDecl |> fst);
              fileUri = env.file.uri;
            };
        None)
            ...
            ~diagnostic:
              {
                ...
                message =
                  Printf.sprintf
                    "The payload of the variant case `%s` of the GraphQL input \
                     union variant `%s` is not a valid GraphQL type. The \
              ...
              message =
                Printf.sprintf
                  "The payload of the variant case `%s` of the GraphQL input \
                   ...
                   needs to be a single type representing a valid GraphQL \
                   type."
                  case.cname.txt (case.typeDecl |> fst);
              fileUri = env.file.uri;
            };
...
and objectTypeFieldsOfRecordFields ~objectTypeName ~env ~schemaState ~debug
  ...
  |> List.filter_map (fun (field : SharedTypes.field) ->
      match
        field.attributes |> getFieldAttributeFromRawAttributes ~schemaState ~env
      with
      | None -> None
             ...
             ~diagnostic:
               {
                 ...
                 message =
                   Printf.sprintf
                     "Field `%s` is has a @gql.field annotation, but is not a \
                      valid GraphQL type. Please change the type of this field \
                      to be a valid GraphQL type, or expose this field via a \
                      custom written resolver instead if you need to transform \
                      it before exposing it in GraphQL."
                     field.fname.txt;
               };
        None
      | Some typ ->
                 ...
                 message =
                   Printf.sprintf
                     "Field `%s` is not a valid GraphQL type. Please change \
                      ...
                      expose this field via a custom written resolver instead \
                      if you need to transform it before exposing it in \
                      GraphQL."
                     field.fname.txt;
               };
        None
    ...
    | Some targetGraphQLType -> (
      ...
      match
        findGraphQLType returnType ~debug
          ~typeContext:(ReturnType {parentTypeName; fieldName = resolverName})
          ~isSubscription:(parentTypeName = "Subscription")
                 ...
                 ~diagnostic:
                   {
                     ...
                     message =
                       Printf.sprintf
                         "Scalars named `t` in a module where the underlying \
                          implementation is not serializable to JSON needs \
                          accompanying `parseValue` and `serialize` functions \
                          for serializing and parsing values for the scalar. \
                          One or both of those functions is not defined in \
                          this module.\n\n\
                          Either define them, or change the implementation of \
                          `t` to use a type that's serializable to JSON \
                          without conversion.";
                   }
          | false, _, _ ->
                 ...
                 ~diagnostic:
                   {
                     ...
                     message =
                       Printf.sprintf
                         "This GraphQL scalar maps to something that requires \
                          custom parsing and serializing. You can either \
                          change this to a valid JSON type, or you can create \
                          a module that defines a `type t` annotated with \
                     ...
                     message =
                       Printf.sprintf
                         "Could not figure out what GraphQL type to attach \
                          ...
                          GraphQL type you want this resolver to be attached \
                          to.";
                   })
        | _, None ->
              ...
              ~diagnostic:
                {
                  ...
                  message =
                    Printf.sprintf
                      "This type is annotated with @gql.inputUnion, but is not \
                       a variant. Only variants can represent GraphQL input \
                       union.";
                }
          | Some Enum ->
            add
              ...
              ~diagnostic:
                {
                  ...
                  message =
                    Printf.sprintf
                      "This type is annotated with @gql.scalar, but is not a \
                       type alias. Only type aliases can represent GraphQL \
                       scalars as of now (but other representations will be \
              ...
              ~diagnostic:
                {
                  ...
                  message =
                    Printf.sprintf
                      "This type is annotated with @gql.interfaceResolver, but \
                       is not a variant. Only variants can represent GraphQL \
                       interface resolvers.";
                })))
